...
// Handles file picking and downloading from OneDrive (Personal and Business)
...
class OneDriveConfiguration {
  private static instance: OneDriveConfiguration;
  ...
  public static getInstance(): OneDriveConfiguration {
    if (!OneDriveConfiguration.instance) {
      OneDriveConfiguration.instance = new OneDriveConfiguration();
    }
    return OneDriveConfiguration.instance;
  }
  ...
  private async getCredentials(): Promise<void> {
    ...
    const onedriveConfig: OneDriveConfig = config.onedrive || {};

    this.clientIdPersonal = onedriveConfig.client_id_personal || '';
    this.clientIdBusiness = onedriveConfig.client_id_business || '';
    this.sharepointUrl = onedriveConfig.sharepoint_url || '';
    this.sharepointTenantId = onedriveConfig.sharepoint_tenant_id || '';
...
async function getToken(
  ...
  const config = OneDriveConfiguration.getInstance();
...
export async function openOneDrivePicker(
  ...
  return new Promise((resolve, reject) => {
    ...
    const params = {
      sdk: '8.0',
      entry: {
        oneDrive: config.getAuthorityType() !== 'organizations' ? { files: {} } : {},
      },
      authentication: {},
      messaging: {
