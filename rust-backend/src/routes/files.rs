    ...
    while let Some(field) = payload.next().await {
        ...
        if field_name == "file" {
            ...
            while let Some(chunk) = field.next().await {
                let chunk = chunk.map_err(|e| AppError::BadRequest(format!("Chunk read error: {}", e)))?;
...
pub fn create_routes(cfg: &mut web::ServiceConfig) {
    cfg.service(
        web::scope("/files")
            ...
            .route("/{id}/process/status", web::get().to(get_file_process_status))
            .route("/{id}/data/content", web::get().to(get_file_data_content))
            .route("/{id}/data/content/update", web::post().to(update_file_data_content))
