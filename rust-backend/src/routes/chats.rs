...
async fn get_chats(
    ...
    let chats = service.get_chats_by_user_id(&auth_user.id, false, skip, limit).await?;
...
async fn get_all_chats(
    ...
    let service = ChatService::new(&state.db);
    let chats = service.get_chats_by_user_id(&auth_user.id, true, 0, 100).await?;
...
async fn get_all_user_chats(
    ...
    let service = ChatService::new(&state.db);
    let chats = service.get_chats_by_user_id(&auth_user.id, false, 0, 1000).await?;
...
async fn create_new_chat(
    ...
    let title = payload.chat.get("title")
        .and_then(|v| v.as_str())
        .map(|s| s.to_string());
...
async fn get_chat(
    ...
    let service = ChatService::new(&state.db);
    let chat = service.get_chat_by_id_and_user_id(&id, &auth_user.id).await?;
...
async fn get_chat_list(
    ...
    let chats = service.get_chat_title_id_list_by_user_id(&auth_user.id, skip, limit).await?;
...
async fn get_user_chat_list_by_user_id(
    ...
    let chats = service.get_chat_list_by_user_id(&user_id, skip, limit).await?;
    Ok(HttpResponse::Ok().json(chats))
}
...
async fn search_user_chats(
    ...
    let chats = service.search_chats_by_user_id(&auth_user.id, &query.text, skip, limit).await?;
    Ok(HttpResponse::Ok().json(chats))
}
...
async fn get_archived_session_user_chat_list(
    ...
    let chats = service.get_archived_chat_list_by_user_id(&auth_user.id, skip, limit).await?;
    Ok(HttpResponse::Ok().json(chats))
}
...
async fn get_chats_by_folder_id(
    ...
    let service = ChatService::new(&state.db);
    let chats = service.get_chats_by_folder_id_full(&folder_id, &auth_user.id).await?;
...
async fn get_chat_list_by_folder_id(
    ...
    let chats = service.get_chat_list_by_folder_id(&folder_id, &auth_user.id, skip, limit).await?;
    Ok(HttpResponse::Ok().json(chats))
}
...
async fn get_user_chat_list_by_tag_name(
    ...
    let chats = service.get_chat_list_by_user_id_and_tag_name(&auth_user.id, &form_data.name, skip, limit).await?;
    Ok(HttpResponse::Ok().json(chats))
}
...
async fn clone_chat_by_id(
    ...
    match chat {
        Some(chat) => {
            let cloned_chat = service.clone_chat(&auth_user.id, chat, form_data.title.clone()).await?;
...
async fn update_chat_folder_id_by_id(
    ...
    let service = ChatService::new(&state.db);
    let chat = service.update_chat_folder(&id, &auth_user.id, form_data.folder_id.clone()).await?;
    let response: ChatResponse = chat.into();
    Ok(HttpResponse::Ok().json(response))
}
...
async fn update_chat_message_by_id(
    ...
    let chat = service.update_chat_message(&id, &message_id, &auth_user.id, &form_data.content).await?;
...
async fn get_chat_tags(
    ...
    match chat {
        Some(chat) => {
            ...
            let tags = if let Some(meta) = chat.meta {
                if let Some(tags_array) = meta.get("tags").and_then(|v| v.as_array()) {
                    tags_array.iter().filter_map(|v| v.as_str().map(String::from)).collect::<Vec<_>>()
                } else {
                    Vec::new()
                }
            } else {
            ...
            let tag_objects: Vec<serde_json::Value> = tags.iter().map(|name| {
                json!({
                    "name": name,
                })
            }).collect();
        ...
        None => {
            Ok(HttpResponse::NotFound().json(json!({"error": "Chat not found"})))
        }
    }
}
...
async fn add_tag_by_id_and_tag_name(
    ...
    let service = ChatService::new(&state.db);
    let chat = service.add_chat_tag(&id, &auth_user.id, &form_data.name).await?;
...
async fn delete_tag_by_id_and_tag_name(
    ...
    let service = ChatService::new(&state.db);
    let chat = service.delete_chat_tag(&id, &auth_user.id, &form_data.name).await?;
