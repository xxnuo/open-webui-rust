...
async fn get_models(
    ...
    for (idx, url) in config.openai_api_base_urls.iter().enumerate() {
        if let Some(key) = config.openai_api_keys.get(idx) {
            ...
            let api_config = config.openai_api_configs.get(idx.to_string())
            ...
            let enabled = api_config.get("enable")
            ...
            // Check if it's Azure
            let is_azure = api_config.get("azure")
                .and_then(|v| v.as_bool())
                .unwrap_or(false);
            ...
            if is_azure {
                    ...
                    .await
                {
                                ...
                                for model in data {
                                    if let Some(model_id) = model.get("id").and_then(|v| v.as_str()) {
                                        ...
                                        if url.contains("api.openai.com") {
                                            let should_skip = ["babbage", "dall-e", "davinci", "embedding", "tts", "whisper"]
                                                .iter()
                    ...
                    Ok(response) => {
                        eprintln!("Failed to fetch models from {}: status {}", url, response.status());
                    }
                    Err(e) => {
...
async fn get_models_by_idx(
    ...
    let key = config.openai_api_keys.get(idx).map(|s| s.as_str()).unwrap_or("");
    ...
    if is_azure {
        ...
        let model_ids = api_config.get("model_ids")
        ...
        .await
    {
        Ok(response) if response.status().is_success() => {
            if let Ok(mut models_response) = response.json::<serde_json::Value>().await {
                ...
                if url.contains("api.openai.com") {
                    if let Some(data) = models_response.get_mut("data").and_then(|v| v.as_array_mut()) {
                        data.retain(|model| {
                            if let Some(model_id) = model.get("id").and_then(|v| v.as_str()) {
                ...
                Err(AppError::InternalServerError("Failed to parse models response".to_string()))
            }
        }
        Ok(response) => {
            let status = response.status();
            let error_text = response.text().await.unwrap_or_default();
            eprintln!("Failed to fetch models from {}: {} - {}", url, status, error_text);
...
pub fn get_openai_endpoint(
    ...
    for (idx, endpoint_url) in config.openai_api_base_urls.iter().enumerate() {
        let api_cfg = config.openai_api_configs.get(idx.to_string())
            .or_else(|| config.openai_api_configs.get(endpoint_url))
            .and_then(|v| v.as_object())
            .cloned()
            .unwrap_or_default();

        let enabled = api_cfg.get("enable")
            .and_then(|v| v.as_bool())
            .unwrap_or(true);
        
        if enabled {
            let endpoint_key = config.openai_api_keys.get(idx)
                .map(|s| s.to_string())
                .unwrap_or_default();
            return Ok((endpoint_url.clone(), endpoint_key, serde_json::json!(api_cfg)));
        }
    }
    
    Err(AppError::NotFound("No enabled OpenAI endpoint configured".to_string()))
}
...
fn get_endpoint_from_cache_or_config(
    ...
    let url_idx = model_item.get("urlIdx")
        .and_then(|v| v.as_u64())
        .or_else(|| {
            payload_obj.get("urlIdx").and_then(|v| v.as_u64())
        })
        .or_else(|| {
    ...
    let selected_idx = if let Some(idx) = url_idx {
        ...
        if idx >= config.openai_api_base_urls.len() {
            return Err(AppError::NotFound(format!("OpenAI endpoint index {} not found", idx)));
        }
        idx
    } else {
        ...
        found_idx.ok_or_else(|| AppError::NotFound("No enabled OpenAI endpoint configured".to_string()))?
    };

    let endpoint_url = config.openai_api_base_urls[selected_idx].clone();
    let endpoint_key = config.openai_api_keys.get(selected_idx)
        .map(|s| s.to_string())
        .unwrap_or_default();
    
    let api_cfg = config.openai_api_configs.get(selected_idx.to_string())
        .or_else(|| config.openai_api_configs.get(&endpoint_url))
        .cloned()
        .unwrap_or(serde_json::json!({}));
    
    tracing::info!("Using endpoint {} (idx: {}) for model {}", endpoint_url, selected_idx, model_id);
...
async fn verify_connection(
    ...
    if is_azure {
        let api_version = api_config.get("api_version")
            .and_then(|v| v.as_str())
            .unwrap_or("2023-03-15-preview");
        
        let auth_type = api_config.get("auth_type")
            .and_then(|v| v.as_str())
            .unwrap_or("bearer");
        
        let mut request_builder = client.get(format!("{}/openai/models?api-version={}", url, api_version));
        ...
        match request_builder.send().await {
            ...
            Err(e) => {
                Err(AppError::InternalServerError(format!("Connection error: {}", e)))
            }
        }
    } else {
...
async fn audio_speech(
    ...
    let openai_idx = config.openai_api_base_urls.iter()
        .position(|url| url.contains("api.openai.com"));
    
    let idx = openai_idx.unwrap_or(0);
    
    if idx >= config.openai_api_base_urls.len() {
        return Err(AppError::NotFound("OpenAI endpoint not configured".to_string()));
    }
    
    let url = config.openai_api_base_urls[idx].clone();
    let key = config.openai_api_keys.get(idx)
        .map(|s| s.to_string())
        .unwrap_or_default();
    ...
    if file_path.exists() {
        return Ok(HttpResponse::Ok()
            ...
            .body(std::fs::read(&file_path).map_err(|e| {
    ...
    match request_builder.send().await {
        ...
        Err(e) => {
            Err(AppError::InternalServerError(format!("Request error: {}", e)))
        }
    }
}
...
async fn embeddings_endpoint(
    ...
    if !config.enable_openai_api {
        return Err(AppError::NotImplemented("OpenAI API is not enabled".to_string()));
    }
...
async fn proxy_request(
    ...
    let request_url = if is_azure {
        ...
        format!("{}/{}?api-version={}", modified_url, request_path, api_version)
    } else {
    ...
    match request_builder.send().await {
        ...
        Err(e) => {
            Err(AppError::InternalServerError(format!("Proxy request error: {}", e)))
        }
    }
}
...
async fn process_streaming_via_socketio(
                                    ...
                                    if let Some(first_choice) = choices.first() {
                                        ...
                                        if let Some(finish_reason) = first_choice.get("finish_reason") {
                                            if !finish_reason.is_null() {
                                                ...
                                                if let (Some(cid), Some(mid)) = (chat_id.as_ref(), message_id.as_ref()) {
    ...
    if should_generate_title && chat_id.is_some() {
        tracing::info!("Triggering title generation for chat: {}", chat_id.as_ref().unwrap());
...
async fn upsert_chat_message(
    ...
    if let Some(chat) = chat_service.get_chat_by_id(chat_id).await? {
        ...
        if let Some(content) = sanitized_message_data.get("content").and_then(|v| v.as_str()) {
        ...
        if let Some(obj) = chat_json.as_object_mut() {
            ...
            if let Some(history_obj) = history.as_object_mut() {
                let messages = history_obj.entry("messages")
                    .or_insert_with(|| serde_json::json!({}));
                
                if let Some(messages_obj) = messages.as_object_mut() {
                    // Get existing message or create new one
                    let existing_message = messages_obj.get(message_id)
                        .and_then(|v| v.as_object())
                        .cloned();
                    ...
                    if let Some(mut existing_msg) = existing_message {
                        ...
                        messages_obj.insert(message_id.to_string(), serde_json::json!(existing_msg));
                    } else {
                        // Insert new message
        ...
        chat_service.update_chat(chat_id, &chat.user_id, update_req).await?;
    }
...
async fn generate_and_update_title(
    ...
    // Get configuration and build request in a scoped block to ensure lock is dropped
    let (endpoint_url, endpoint_key, title_payload) = {
        let config = state.config.read().unwrap();
        ...
        let messages_for_title: Vec<_> = messages.iter()
            .rev()
            .take(2)
            .rev()
            .cloned()
        ...
        // Get endpoint
        let (endpoint_url, endpoint_key, _) = get_openai_endpoint(&config, model_id)?;
        ...
        (endpoint_url, endpoint_key, title_payload)
...
pub async fn handle_chat_completions(
    ...
    let model_item = payload_obj.as_object_mut()
        .and_then(|obj| obj.remove("model_item"))
        .unwrap_or(serde_json::json!({}));
    
    // Extract Socket.IO streaming metadata from top-level (frontend sends these at root level)
    let session_id = payload_obj.get("session_id")
        .and_then(|v| v.as_str())
        .map(String::from);
    let chat_id = payload_obj.get("chat_id")
        .and_then(|v| v.as_str())
        .map(String::from);
    let message_id = payload_obj.get("id")
        .and_then(|v| v.as_str())
        .map(String::from);
    
    // Extract background_tasks for title generation, etc.
    let background_tasks = payload_obj.as_object_mut()
        .and_then(|obj| obj.remove("background_tasks"))
        .unwrap_or(serde_json::json!({}));
    
    let should_generate_title = background_tasks.get("title_generation")
        .and_then(|v| v.as_bool())
        .unwrap_or(false);
    
    // Extract messages for title generation before removing from payload
    let messages = payload_obj.get("messages")
        .and_then(|v| v.as_array())
        .cloned()
        .unwrap_or_default();
    ...
    let metadata = payload_obj.as_object_mut()
        .and_then(|obj| obj.remove("metadata"))
        .unwrap_or(serde_json::json!({}));

    tracing::debug!("Chat completion request - model_id: {}, model_item: {}", model_id, serde_json::to_string(&model_item).unwrap_or_default());

    // Check if this is a direct connection request (Python: if not model_item.get("direct", False))
    let is_direct = model_item.get("direct")
        .and_then(|d| d.as_bool())
        .unwrap_or(false);
    
    tracing::debug!("is_direct: {}, session_id: {:?}, model_id: {}", is_direct, session_id, model_id);
    ...
    let (url, key, api_config) = if is_direct && config.enable_direct_connections {
        ...
        let direct_url = model_item.get("url")
            .and_then(|u| u.as_str())
            .filter(|s| !s.is_empty());
        
        let direct_key = model_item.get("key")
            .and_then(|k| k.as_str())
            .unwrap_or("");

        if let Some(url_str) = direct_url {
            tracing::info!("Using direct connection with explicit URL: {} for model {} by user {}", url_str, model_id, auth_user.user.email);
            
            let item_config = model_item.get("config")
                .cloned()
                .unwrap_or(serde_json::json!({}));
            ...
            tracing::debug!("Looking up direct connection from user settings with urlIdx: {}", url_idx);
            ...
            let connection_config = configs.get(&url_idx)
                .or_else(|| configs.get(connection_url))
                .cloned()
                .unwrap_or(serde_json::json!({}));
            
            tracing::info!("Using direct connection from user settings: {} (idx: {}) for model {} by user {}", 
                connection_url, idx, model_id, auth_user.user.email);
            
            (connection_url.to_string(), connection_key.to_string(), connection_config)
        }
    ...
    match request_builder.json(&payload_obj).send().await {
        Ok(response) if response.status().is_success() => {
            ...
            let is_stream = payload_obj.get("stream").and_then(|v| v.as_bool()).unwrap_or(false);
            
            tracing::debug!("Response content-type: {}, stream param: {}", content_type, is_stream);
            ...
            if is_stream && content_type.contains("text/event-stream") {
                ...
                let use_socketio = session_id.is_some() && chat_id.is_some() && message_id.is_some() 
                ...
                if use_socketio {
                    ...
                    tracing::info!("🔵 Using Socket.IO for real-time streaming to session: {:?}", session_id);
                ...
                if let Ok(json_response) = response.json::<serde_json::Value>().await {
                    ...
                    Err(AppError::InternalServerError("Failed to parse response".to_string()))
                }
            }
        }
        Ok(response) => {
        ...
        Err(e) => {
            ...
            Err(AppError::InternalServerError(
                format!("Error calling OpenAI API: {}", e)
            ))
        }
    }
}
