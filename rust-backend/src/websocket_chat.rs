                ...
                WsMessage::Text(text) => {
                    ...
                    match serde_json::from_str::<serde_json::Value>(&text) {
                        Ok(payload) => {
                            ...
                            if let Err(e) = process_chat_stream(
                                &state_clone,
                                payload,
                                &mut session,
                            ).await {
...
async fn process_chat_stream(
    ...
    // Extract metadata
    let chat_id = payload.get("metadata")
        .and_then(|m| m.get("chat_id"))
        .and_then(|v| v.as_str())
        .map(String::from);
    let message_id = payload.get("metadata")
    ...
    if is_stream {
        ...
        tracing::info!("🏁 Streaming finished. Total content length: {}", accumulated_content.len());
        ...
        if let (Some(ch_id), Some(msg_id)) = (chat_id, message_id) {
            if !accumulated_content.is_empty() {
                ...
                if let Ok(Some(chat)) = chat_service.get_chat_by_id(&ch_id).await {
                            ...
                            if let Some(messages_obj) = messages.as_object_mut() {
                                ...
                                if let Some(existing_msg) = messages_obj.get(&msg_id) {
                                    ...
                                    if let Some(existing_obj) = existing_msg.as_object() {
                                        ...
                                        messages_obj.insert(msg_id.clone(), serde_json::Value::Object(merged));
                                    }
                                } else {
                                ...
                                history_obj.insert("currentId".to_string(), serde_json::Value::String(msg_id.clone()));
                            }
                        }
                    }
                    ...
                    let update_result = chat_service.update_chat(&ch_id, &chat.user_id, UpdateChatRequest {
                        title: None,
                        chat: Some(chat_json),
                        folder_id: None,
                        archived: None,
                    ...
                    match update_result {
                        Ok(_) => tracing::info!("✓ Saved chat message to DB: chat_id={}, msg_id={}, content_len={}", ch_id, msg_id, accumulated_content.len()),
